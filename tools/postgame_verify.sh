...
# If a recorder is still running, skip the newest (likely in-progress)
if pgrep -x ffmpeg >/dev/null; then
  IFS=$'\n' files=( $(ls -t output/soccer/full/*.mp4) )
  files=( "${files[@]:1}" )
fi
...
for f in "${files[@]}"; do
  ...
  if [[ -n "$v" && -n "$a" && "$d" != "" ]] && (( ${d%.*} >= 30 )); then
    printf "✅ %s  (video=%s, audio=%s, duration=~%.1fs)\n" "$f" "$v" "$a" "$d"
    ((ok++))
  else
