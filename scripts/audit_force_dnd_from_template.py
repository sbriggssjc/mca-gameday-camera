...
ORDINAL = {'1':1,'2':2,'3':3,'4':4,'1st':1,'2nd':2,'3rd':3,'4th':4,'first':1,'second':2,'third':3,'fourth':4}
WORD_NUM = {
...
def as_int_or_word(s, allow_goal=False):
    ...
    parts = s1.split()
    if len(parts)==2 and parts[0]=='twenty' and parts[1] in WORD_NUM: return 20 + WORD_NUM[parts[1]]
    return None

def clip_id_from_str(s): return _clip_id_from_pathish(s)
...
def main():
    if not plays_path.exists():
        print(f"[err] missing {plays_path}"); sys.exit(1)
    if not tmpl_path.exists():
        print(f"[err] missing {tmpl_path}"); sys.exit(1)
